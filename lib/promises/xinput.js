...
import { XInput as lib } from "../ffi/XInput.js";
...
async function enable(enable){
  ...
  await promisify(lib.XInputEnable.async)(enable ? 1 : 0);
}
...
async function getBatteryInformation(option = {}){
  ...
  //Explicit error
  shouldIntegerWithinRange(options.dwUserIndex, 0, CONTROLLER.XUSER_MAX_COUNT.XInput - 1);
...
async function getKeystroke(option = {}){
  ...
  //Explicit error
  if (
    !( isIntegerWithinRange(options.dwUserIndex, 0, CONTROLLER.XUSER_MAX_COUNT.XInput - 1) || 
    ...
    throw new Failure("Expecting an integer between the specified range or equals to 255 !", { 
      ...
      info: { 
        ...
        range: [0, CONTROLLER.XUSER_MAX_COUNT.XInput - 1], 
...
async function waitForGuideButton(option = {}){
  ...
  const code = await promisify(lib.XInputWaitForGuideButton.async)(options.dwUserIndex, options.dwFlags, {});
  if (code !== 0) throw new Failure(...errorLookup(code));
}
...
async function getBaseBusInformation(option = {}){
  ...
  const code = await promisify(lib.XInputGetBaseBusInformation.async)(options.dwBusIndex, Information);
  if (code !== 0) throw new Failure(...errorLookup(code));
