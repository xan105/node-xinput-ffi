...
function GetBatteryInformation(option = {}){
  ...
  const code = lib.XInputGetBatteryInformation(options.gamepadIndex, options.devType, battery.ref()); 
...
function GetCapabilities(option = {}){
  ...
  const code = lib.XInputGetCapabilities(options.gamepadIndex, options.flags, capabilities.ref());
  if (code !== 0) throw new Failure(...translate.ERROR(code));
...
function GetCapabilitiesEx(option = {}){
  ...
  const code = lib.XInputGetCapabilitiesEx(1, options.gamepadIndex, 0, capabilitiesEx.ref());
  if (code !== 0) throw new Failure(...translate.ERROR(code));
...
function getState(option = {}){
  ...
  const code = lib.XInputGetState(options.gamepadIndex, state.ref());
  if (code !== 0) throw new Failure(...translate.ERROR(code));
...
function getButtonsDown(option = {}){
  ...
  const state = getState({ gamepadIndex: options.gamepadIndex, translate: true });
