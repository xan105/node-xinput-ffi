...
async function listDevices() {
  ...
  for (const PNPentity of PNPentities) {
    ...
    if (PNPentity.deviceID.includes("VID_") && PNPentity.deviceID.includes("PID_")) { 
      ...
      const VID = Number("0x" + getVID(PNPentity.deviceID));
      const PID = Number("0x" + getPID(PNPentity.deviceID));
      ...
      { 
        if (!(hardwareID[VID]?.name && hardwareID[VID]?.controller[PID])) continue;
